...
<!--
今天录下来的东西，十年后还安全吗？在量子密码这个角度来看安全不是只看今天能不能破解，还要看密文被保存以后会发生什么。今天我们就用三篇论文看一件事：协议迁移到量子密码环境下还能不能保障安全。
-->
...
<div class="deck-slide">
  <h2>HNDL：先收集，未来再解密</h2>
  <p class="slide-lede">”十年后还安全吗”对应的攻击已经从今天开始了：攻击者不需要立刻破解，只需要先保存密文，等未来工具成熟后再回头解。</p>
  <div class="timeline">
    ...
    <div class="timeline-step">
      ...
      <div class="step-title text-pq">迁移时分析目前的协议能扛多久</div>
      <div class="step-body">迁移问题要回答旧密文、旧密钥和旧握手还能暴露多久。</div>
    </div>
  </div>
  <div class="takeaway quantum">这个攻击模式叫 HNDL：Harvest Now, Decrypt Later。它把量子迁移的时间压力提前到今天——不是未来被破，是现在就要让协议状态能扛住未来的解密能力。</div>
</div>

<!--
后量子迁移的紧迫性来自今天已经能被存走的密文，量子计算机成熟只是后半场。
HNDL 可以翻成一句人话：先收割，未来再解密。它描述的是一种时间上的攻击策略。
攻击者现在只需要监听和保存；未来如果经典握手材料能被恢复，旧通信就会被重新打开。
今天在讲完背景概念后，用三篇论文分别分析哪些协议。
-->
...
<div class="deck-slide roadmap-slide">
  <h2>协议的迁移&形式化验证</h2>
  <p class="slide-lede">HNDL 提出威胁问题，三篇论文对应三个具体协议：消息协议、VPN 握手、网络层隧道。希望解决同一件事 —— 协议状态怎样在未来泄露下继续安全。</p>
  <div class="roadmap-grid">
    <div class="roadmap-card pq">
      <div class="case-label">Case A</div>
      <h3>PQ3</h3>
      <p>看 iMessage 对话里的设备状态、消息密钥和不断前进的密钥链。</p>
      <strong>消息状态</strong>
    </div>
    <div class="roadmap-card hybrid">
      <div class="case-label">Case B</div>
      <h3>Hybrid-WireGuard</h3>
      <p>看两条 VPN 握手消息里，身份、方向和密钥派生怎样重新绑定。</p>
      <strong>握手状态</strong>
    </div>
    <div class="roadmap-card proto">
      <div class="case-label">Case C</div>
      <h3>PQConnect</h3>
      <p>看网络层隧道怎样发现对端、建立密钥，并控制包级擦除窗口。</p>
      <strong>隧道状态</strong>
    </div>
  </div>
  ...
  <div class="takeaway formal">主线任务：迁移后的状态机，能不能挡住未来泄露和网络攻击。</div>
</div>

<!--

三篇论文的共同点是都在问迁移后的协议状态能不能继续安全。

PQ3 是消息状态，Hybrid-WireGuard 是握手状态，PQConnect 是网络层隧道状态。

状态怎么变，泄露何时发生，有没有坏路径。

为什么这个问题会先落到公钥握手上？下一页用量子攻击的影响范围说明。
...
<div class="deck-slide quantum-compare">
  <h2>公钥被冲击：协议迁移迫在眉睫</h2>
  <p class="slide-lede">量子计算最直接影响的是现有公钥握手和身份认证的可信前提。</p>
  <div class="slide-grid">
    <div class="algorithm-card shor-card">
      ...
      <div class="compact-list">
        ...
        <p><strong>本场主线：</strong>公钥握手迁移以后，协议状态是否仍然安全。</p>
      </div>
    </div>
    <div class="algorithm-card grover-card">
      ...
      <p class="algorithm-plain">它把无结构搜索从 N 次量级降到约 √N 次；直观上等于吃掉一部分对称安全余量。</p>
      <div class="compact-list">
        <p><strong>它影响什么：</strong>AES、SHA、MAC、KDF 这类对称组件的安全余量。</p>
        <p><strong>常见处理：</strong>调大密钥或输出长度，给搜索成本留更多空间。</p>
        <p><strong>本场位置：</strong>这是重要背景；今天三篇论文的主要矛盾在公钥握手迁移。</p>
      </div>
    </div>
  </div>
  <div class="takeaway quantum">公钥握手一改，迁移就变成协议状态机的重写。后面三篇论文都在回答：重写以后，状态还安全吗。</div>
</div>

<!--
这里停一下，先讲 Shor 和 Grover 是什么。
...
<div class="deck-slide pqc-assembly">
    ...
    <div class="assembly-side">
      ...
      <div class="part-list">
        <div class="part-item pq">
          ...
          <span>它回答“能不能产生抗量子共享秘密”，不自动回答协议怎样使用。</span>
        </div>
  ...
  <div class="takeaway">迁移的对象是协议状态机：新材料放错位置，算法再强也救不了协议结论。</div>
</div>
...
<div class="deck-slide formal-intro-slide">
  <h2>状态分支太多，工具帮我们找坏路径</h2>
  <p class="slide-lede">上一页的问题不能只靠正常流程图回答。验证工具会系统搜索模型允许的走法，包括重放、乱序、插入消息和不同泄露时刻。</p>
  <div class="formal-check-layout">
    <div class="model-card">
      <div class="paper-kicker">模型里先写清三件事</div>
      <div class="model-row">
        <div class="row-index proto">1</div>
        <div>
          <strong>一步允许发生的动作 <em>rule</em></strong>
          <p>发送、接收、生成密钥、删除密钥。</p>
        </div>
      </div>
      <div class="model-row">
  ...
  <div class="takeaway formal">形式化验证把“人工容易漏的分支”变成工具要搜索的坏路径。</div>
</div>
...
<div class="deck-slide kem-dh-slide">
  ...
  <div class="takeaway hybrid">KEM 接替 DH 时，协议必须重新写清：谁接收封装、谁证明身份、密文和共享秘密进入哪一轮状态。</div>
</div>
...
<div class="section-slide">
  ...
  <div class="section-points">
    ...
    <p>论文把未来经典秘密泄露写进 Tamarin 模型，检查设备到设备消息状态还能保护到哪里。</p>
